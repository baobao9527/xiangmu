...
import com.mo008.dtos.RealtimParamDto;
...
public class RealtimeController extends Controller {
    ...
    public void list() {
        ...
        if (reqOptional.isPresent()) {
            ...
            int page = req.page;
            SqlNode sql = SqlKit.sqlNode("deviceData.easyui.datagrid");
            // 替换SQL为分表
            SqlNode newSql = SubmeterKit.convertSqlNode(sql);

            final List<Triple<String, Condition, Object>> customParams = req.params;
            StringBuilder where = DTDao.appendWhereSql(params, newSql, customParams);
            String carNo = getPara("carNo");
            if (!Strings.isNullOrEmpty(carNo)) {
                where.append(" AND mci.car_no like ? ");
                params.add(Strs.like(carNo));
            }
            ...
            if (!Strings.isNullOrEmpty(driverCompany)) {
                where.append(" AND md.work_unit like ? ");
                params.add(Strs.like(driverCompany));
            }
            /**
             *添加生产厂商查询
             */
            String terminal = getPara("terminal");
            if (!Strings.isNullOrEmpty(terminal)) {
                where.append(" AND mci.terminal like ? ");
                params.add(Strs.like(terminal));
            }
            ...
            if (startDate!=null && !"".equals(startDate)){
                queryStartDate = DateTime.parse(startDate, ymdFormatter).millisOfDay().withMinimumValue();
                params.add(queryStartDate.toDate());
                where.append(" AND mdd.report_time > ? ");
            }
            if (endDate!=null && !"".equals(endDate)){
                queryEndDate = DateTime.parse(endDate, ymdFormatter).millisOfDay().withMaximumValue();
...
//            final int year = queryStartDate.getYear();
            ...
            if (noQualified > 0) {
                ...
                switch (noQualified) {
                    case 1:
                        where.append(" AND mdd.flag_mileage = ?");
                        params.add(true);
                        break;
                    case 2:
                        where.append(" AND mdd.flag_speed = ?");
                        params.add(true);
                        break;
                    case 3:
                        where.append(" AND mdd.temperature_1_flag = ?");
                        params.add(true);
                        break;
                    case 4:
                        where.append(" AND mdd.temperature_2_flag = ?");
                        params.add(true);
                        break;
                    case 5:
                        where.append(" AND mdd.temperature_3_flag = ?");
                        params.add(true);
                        break;
                    case 6:
                        where.append(" AND mdd.temperature_4_flag = ?");
                        params.add(true);
                        break;
                    case 7:
                        where.append(" AND mdd.pressure_flag = ?");
                        params.add(true);
                        break;
                    case 8:
                        where.append(" AND mdd.urea_temperature_flag = ?");
                        params.add(true);
                        break;
                    case 9:
                        where.append(" AND mdd.urea_position_flag = ?");
                        params.add(true);
                        break;
                    default:
                        break;
            ...
            if (!Strings.isNullOrEmpty(areaSn)) {
                where.append(" AND mci.area_sn like ?");
                params.add(areaSn + "%");
            }
            ...
            final Page<Record> paginate = Db.paginate(page,
                    pageSize,
                    "SELECT t. * ",
                    " FROM (" + sql.selectSql + where.toString() + ") t  ORDER BY report_time DESC",
                    params.toArray());
            DataGridRsp.Builder builder = new DataGridRsp.Builder();
            final List<Record> realtimRecords = paginate.getList();
            final List<String> onlineDeviceCodes = RedisService.getInstance().onlineDeviceCodes();
            for (Record realtimRecord : realtimRecords) {
                final String deviceCode = realtimRecord.getStr("device_code");
                realtimRecord.set("online", onlineDeviceCodes.contains(deviceCode));
            }
            builder.rows(realtimRecords).total(paginate.getTotalRow());
    ...
    public void update() {
        String datas = getPara("prows");
        if (Strings.isNullOrEmpty(datas)) {
            renderJson(EuiDataGrid.EMPTY_DATAGRID);
        } else {
            List<DeviceData> deviceDatas = Lists.newArrayList();
            final List<RealtimParamDto> realtimParamDtos = JSON.parseArray(datas, RealtimParamDto.class);
            for (RealtimParamDto realtimParamDto : realtimParamDtos) {
                if (realtimParamDto.getId() <= 0) {
                    continue;
                }
                DeviceData deviceData = DeviceData.dao.findByIdMore(realtimParamDto.getId());
                if (deviceData == null) {
                    continue;
                }
                if (deviceData.getReportTime().getTime() > realtimParamDto.getTime().getTime()) {
                    deviceDatas.add(deviceData);
                }
            }

            final List<String> onlineDeviceCodes = RedisService.getInstance().onlineDeviceCodes();
            for (DeviceData realtimRecord : deviceDatas) {
                final String deviceCode = realtimRecord.getStr("device_code");
                realtimRecord.put("online", onlineDeviceCodes.contains(deviceCode));
            }

            renderAjaxSuccess(deviceDatas);
        }
    ...
    public void export() {
        ...
        if (reqOptional.isPresent()) {
            ...
            XSSFWorkbook wb = writeExcel(realtimRecords);
    ...
    private XSSFWorkbook writeExcel(List<Record> list) {
        ...
        cellTiltle.setCellValue("车辆数据导出");
...
//遍历每个对象
    ...
    private String encodeFileName() {
        try {
            return new String("车辆数据导出.xlsx".getBytes(StandardCharsets.UTF_8), "ISO8859-1");
