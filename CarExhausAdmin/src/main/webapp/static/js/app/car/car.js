...
var _gridlist, url = {
    ...
    terminallist: g.ctx + 'device/home/bindlist',
    terminal: g.ctx + 'static/views/car/terminal.html',
...
$(function () {
    ...
    $('#a_add_terminal').attr('onclick', 'CarInfo.addTerminal();');
        ...
        if (grid.getSelectedRow()) {
            ...
            if (row.length == 1) {
                this.window = top.$.hDialog({
                    ...
                    submit: function () {
                        if (top.$('#deviceGrid').datagrid('getSelected')) {
                            var deviceId = top.$('#deviceGrid').datagrid('getSelected').id;
                            self.saveTerminal(grid.getSelectedRow().id, deviceId);
                            return false;
                        } else {
                            top.$.messager.alert('温馨提示', '未选中终端设备！', 'warning');
                            return false;
                        }
                    }
                });
            } else {
                top.$.messager.alert('温馨提示', '只能选中一辆车管理终端！', 'warning');
            }
        } else {
