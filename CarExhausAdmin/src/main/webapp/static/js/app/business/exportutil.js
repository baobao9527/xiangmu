        ...
        var hDialog = top.$.hDialog({
            ...
            submit: function () {
                ...
                top.$('#field_list :checked').each(function () {
                    var v = $(this).val();
                    //selectedFields += v.split('|')[0] + ' ' + v.split('|')[1] + ","; //暂时取消用户自定义标题的显示，调用分页会有问题。
                ...
                if (!where)
                    where = "";
                window.open('../ajax/ExportExcel.aspx?tableName=' + tableName + '&sortField=' + sortField + '&fields=' + selectedFields + '&filters=' + where);
            }
        });
