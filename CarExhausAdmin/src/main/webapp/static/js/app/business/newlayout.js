...
var onlyOpenTitle = "平台数据监控";
var openTabs = 10; //允许打开的窗口数量
var errorUrl = 'Error/Error404.html';
$(function () {
    if (_menus) {
        initNav();
        tabClose();
        addTab("平台数据监控", g.ctx +"monitoring/realtime", "icon-desktop");
    } else {
        $.messager.alert("系统提示", "<font color=red><b>您没有任何权限，请联系管理员!</b></font>", "warning", function () {
            location.href = g.ctx +'logout';
        });
    }
...
function initNav() {
    ...
    $('#tabs').tabs({
        tools: [{
            ...
            handler: function () {
                addTab("我的工作台", g.ctx + "dashboard", "icon-desktop");
                return false;
            }
        },
            {
    ...
    addNav: function(data) {
        $.each(data, function(i, sm) {
            ...
            $.each(sm.children, function(j, o) {
                ...
                menulist += '<li><div><a ref="' + o.id + '" href="#" rel="' + g.ctx + o.navigateUrl + '" ><span  iconCls="' + cssIcon + '" class="icon ' + cssIcon
...
var MenuButton = {
    ...
    init: function() {
        ...
        $('#wnav').css({ 'float': 'left', 'width': '100%', 'height': '30px', 'padding': '3px 0px 0px 20px', 'background': '#6ABEFA url('+g.ctx+'static/images/datagrid_title_bg.png)' });

        if (theme == "gray") {
            $('#wnav').css('background', 'url('+g.ctx+'static/easyui/themes/gray/images/tabs_enabled.gif)');
        }
...
function closeTableWithTitle(subtitle){
    var $tabs = $('#tabs');
...
var editMyPass = function () {
    top.$('#w').hWindow({
        width: 300, height: 210, title: '修改密码', iconCls: 'icon-key', html: htmlEditPassowrd, submit: function () {
            if ($('#txtNewPassword').validatebox('isValid')) {
                $.ajaxtext(g.ctx + '/changepwd', "newPassword=" + $('#txtNewPassword').val() + '&oldPassword=' + $('#txtOldPassword').val(), function (msg) {
                    if (msg == '1') {
